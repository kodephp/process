...
> 自 v5.5.1 起「判归属 + 落盘」跑在同一把非阻塞排他 `flock` 里（锁加在 pid 文件本身，open 模式 `'c'`
> 不截断，写盘用持锁的那只手写）：先后两次启动由「文件里写着存活的 pid」拒绝，**同时**敲两次由
> 「另一进程正握着锁」拒绝 —— 只判活挡不住后者，因为判定那一刻对方本来就还没写。
...
### v5.5.1
- **pid 文件互斥现在真的锁得住「同时敲两次」。** v5.5.0 的 `claimPidFile()` 只做「读 → 判 → 返回」，写盘发生在它返回之后、由 `file_put_contents()` 另开一个 fd 完成 —— 两步之间没有任何互斥。实测在同一台机器上同时敲两次 `php kode process:start`：两边都在对方落盘之前读到「没人占用」，于是真起出**两套**守护进程（两个 master 各带 worker），而 pid 文件里只留下其中一个 —— 另一套从状态表上永久隐形，`stop`/`reload` 都摸不到它。判活也挡不住这种交错，因为判定那一刻对方本来就还没写。
- 现在「判归属 + 落盘」跑在同一把**非阻塞排他 flock** 里（新增 `withPidLock()`，判定本体拆成 `assertPidFileUnowned()`）：锁加在 pid 文件本身而不是旁边的 `*.lock`（路径只有一份，读侧与测试拿 `getPidFile()` 就能占到同一个锁），open 用 `'c'`（不存在就建、存在**不截断** —— 截断会把别人的占位抹成空文件，而空文件按「未占用」处理，等于用一次 open 把活着的实例赶走），写盘用**持锁的那只手写**（`ftruncate` + `fwrite`；另开 fd 等于绕过自己的锁）。抢不到锁立刻抛「另一个进程正在启动并占用 …（并发启动）」，不阻塞 supervise 入口；打不开该路径同样抛（别让「写不下去」伪装成「没人占用」）。
- 新增两例回归：`testRefusesToClaimWhileAnotherProcessHoldsThePidFile`（别人握着锁时 claim 必须拒绝、锁放掉后必须能照常认领、成功路径不得把锁留在自己手里）、`testWriteRefusesWhileAnotherProcessHoldsThePidFile`（落盘那一步也必须在锁内，别人握着锁时一个字节都不许动）。三条腿各配变异验证：去掉 flock 判定、把落盘挪出锁、open 改成 `'w'` —— 每一种都必须有对应用例变红。

### v5.5.0
- **`Daemon` 的 pid 文件从此是互斥占位。**（**并发启动除外**：这一版只挡住「先后两次」，同时敲两次仍会叠套，v5.5.1 才补上锁 —— 见上。）`run()` 此前无条件 `file_put_contents()`，第二次启动会把第一代的 pid 覆盖成自己的 —— 而 pid 文件是「谁在跑」的唯一来源，于是第一代守护进程（连同它的 worker 子进程）永久查不到、收不到停止/重载信号，却还在跑同一份资源；`kode process start` 每跑一次就多叠一套。现在新增 `claimPidFile()` 一份判据：文件里写着**存活且非本进程**的 pid 就抛 `ProcessException` 拒绝启动并原样保留文件，失效 pid／内容读不出 pid／文件不存在一律接管。判定排在 `daemonize()` **之前**（守护化后父进程已 exit，在那之后失败等于「静默启动失败」）。
