...
> ⚠️ **多进程下会重复执行**：`Timer::cron()` / `Crontab` 是**按进程**隔离的静态注册表。在 master-worker
> 多进程（或水平扩展多机）下，每个 worker 都会各自注册并触发同一表达式，导致每个调度时刻
> **重复执行 N 次**、且 worker 崩溃即丢失。多进程本身不解决这个问题。需要集群内「至多执行一次」时，
> 用 `Kode::cronCluster()`（每任务分布式锁）或 `Kode::tickCronOnLeader()`（Leader 选举）；
> 持久/可重试的活儿交给 `Kode::queue()`（Redis 队列）。详见 README「多进程定时任务的重复执行问题与选型」。
