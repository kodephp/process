...
|---------|-----:|-----:|----------:|----:|
| Kode\[swoole\] | 4,034,536 | 3,432,412 | 2,294,841 | 1,746,782 |
| Kode\[sysvshm\] | 1,387,790 | 2,099,787 | 613,983 | 20,765 |
| 原生 Swoole\Table | 9,259,358 | 10,113,093 | 14,581,276 | N/A* |

`*` `Swoole\Table` **无原生 CAS**；本库 `SwooleTable` 后端用锁实现 `cas`（见上 Kode\[swoole\] 行）。

观察：
- 本库 `swoole` 后端相对原生 `Swoole\Table` 的吞吐：set 约 40%、get 约 34%、increment 约 16%。差额来自本库统一的 `TableInterface` 封装：`set`/`get` 走 `v` 列序列化、`increment` 经 `incr` + 封装、`cas` 因 `Swoole\Table` 无原生 CAS 故以锁实现（无原生对照）。这是「多后端可互换、统一语义、支持 TTL 与软件 CAS」所付的合理代价。
...
|---------|----:|------:|-----------|
| Kode\[swoole\] | 40,004 | 504,045 | OK |
| Kode\[sysvshm\] | 40,004 | 326,942 | OK |
| 原生 Swoole\Table | 40,004 | 2,842,628 | OK |
...
- 原生 `Swoole\Table` 跨进程吞吐约为本库 `swoole` 后端的 **5.6 倍** —— 差额即本库适配层的锁 + 序列化开销（与单进程微基准的差距一致）。若你的场景对共享表吞吐极度敏感且已装 Swoole，本库 `swoole` 后端的绝对数字（$5\times10^5$ ops/s 量级）已非常可观。
