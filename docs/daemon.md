...
pid 文件回答的是**唯一**一个问题：「这一路守护进程是谁在跑」。因此它的规则只有五条，
归属判定只有一份 = `Daemon::assertPidFileUnowned()`（`run()` 在守护化之前经 `claimPidFile()` 调一次、
`writePidFile()` 在落盘之前再调一次），两条调用都跑在 `withPidLock()` 的同一把锁里：

| pid 文件现状 | 判定 | 依据 |
|------|------|------|
| **另一个进程正握着这把锁** | 抛 `ProcessException` 拒绝启动，理由点名「并发启动」 | 见下方「锁为什么必须罩住落盘」 |
...
- **判定存活用 `Process::isProcessAlive()`，EPERM 也算活着。** 查不到的进程一律按「别抢」处理，
  ...
  管理端发信号前请核对 pid）。
- **锁罩住「判 + 落盘」两步，而不是只罩判定。** v5.5.0 只做「读 → 判 → 返回」，写盘在它返回之后
  由 `file_put_contents()` 另开一个 fd 完成 —— 两步之间没有互斥。实测同时敲两次 `kode process:start`：
  两边都在对方落盘之前读到「没人占用」，于是真起出**两套**守护进程，而 pid 文件里只留下其中一个，
  另一套从此查不到、收不到停止/重载信号。判活也挡不住这种交错（判定那一刻对方本来就还没写）。
  现在写盘用的是**持锁的那只手写**（`ftruncate` + `fwrite`），因为另开 fd 等于绕过自己的锁。
- **锁加在 pid 文件本身，open 模式是 `'c'`。** 不引入 `*.lock` 第二个路径（读侧与测试拿 `getPidFile()`
  就能占到同一个锁），`'c'` 表示「不存在就建、存在则**不截断**」—— 用 `'w'` 打开会把别人的占位抹成
  空文件，而空文件按「未占用」处理，等于用一次 open 把活着的实例赶走。取锁是非阻塞的（`LOCK_EX | LOCK_NB`）：
  正在启动的那一位马上就会放锁，宁可回一句「并发启动，请重试」，也不能让两条命令互相等成看不出原因的挂起。
...
因此 `kode process start` 重复执行**不会**叠出第二套守护进程：先后两次由「文件里写着存活的 pid」拒绝，
同时敲两次由「另一进程正握着锁」拒绝，两种拒绝都点名占位的一方。
...
`exceedsRestartBudget(int $slot)`、`claimPidFile()`（在锁内做一次 `assertPidFileUnowned()`，归属判定唯一一份）、
`writePidFile()`（同一把锁内判定 + 落盘）、`cleanup()`（只删自己那份）。
