...
**直连地址由运行时在交付请求时盖。** 请求对象不知道自己属于哪条连接，而对端地址在
`accept()` 那一刻就已经在手上：三个运行时各自负责把它写进来（Native 取连接的对端名并去端口、
Swoole 取 `server['remote_addr']`、Workerman 取连接的 `getRemoteIp()`），HTTP/2 的每个流
再从父连接继承同一份。因此 `$request->ip()` 与 `$_SERVER['REMOTE_ADDR']` 在三种运行时下
同值；自己 `new` 出来的请求（单测、内部转发）没有对端，返回空串。

空串不是「本机」，而是「不知道」。凡是拿它做**按来路 IP 分桶**的地方（限流、登录防爆破、
审计溯源），都要先确认它非空——把空串当键用，等于把所有客户端折进同一个桶：
一个人刷失败次数，全站一起被锁。

```php
$request->setRemoteAddress('203.0.113.7');   // 自定义传输层/内部转发时补上对端
Request::ipOf('[::1]:9527');                 // 'ip:port' / '[ipv6]:port' → 纯地址（静态纯函数）
```
